module.exports = {
    ...
    start: async(killua, m, { commands, args, prefix, text, toUpper }) => {
        ...
        if (args[0]) {
            ...
            else data.push(`*Comando:* ${prefix}${cmd.name.replace(/^\w/, c => c.toUpperCase())}`)
            if (cmd.alias) data.push(`*Alias:* ${cmd.alias.join(", ")}`)
            if (cmd.use) data.push(`*Uso:* ${cmd.use}`);
            if (cmd.desc) data.push(`*Descripción :* ${cmd.desc}\n`)
            if (cmd.example) data.push(`*Ejemplo :* ${cmd.example.replace(/%prefix/gi, prefix).replace(/%command/gi, cmd.name).replace(/%text/gi, text)}`)
            return m.reply(`*Información de ${cmd.name.replace(/^\w/, c => c.toLowerCase())}*\n\n${data.join("\n")}`)
        } else {
