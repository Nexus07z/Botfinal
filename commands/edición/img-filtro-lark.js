...
module.exports = {
    name: "img-filtro-lark",
    alias: ["imgflark"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro lark.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/lark", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/lark", { img: isUrl(text)[0] }, "apikey"), "", m)
