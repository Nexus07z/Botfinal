...
module.exports = {
    name: "img-filtro-nashville",
    alias: ["imgfnashville"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro nashville.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/nashville", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/nashville", { img: isUrl(text)[0] }, "apikey"), "", m)
