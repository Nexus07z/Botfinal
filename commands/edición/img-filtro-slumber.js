...
module.exports = {
    name: "img-filtro-slumber",
    alias: ["imgfslumber"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro slumber.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/slumber", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/slumber", { img: isUrl(text)[0] }, "apikey"), "", m)
