...
module.exports = {
    name: "img-filtro-toaster",
    alias: ["imgftoaster"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro toaster.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/toaster", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/toaster", { img: isUrl(text)[0] }, "apikey"), "", m)
