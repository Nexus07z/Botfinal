...
module.exports = {
    ...
    alias: ["imgtorainbow"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con estilo arcoiris.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/creator1/rainbow", { img: link }, "apikey"), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/creator1/rainbow", { img: isUrl(text)[0] }, "apikey"), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
