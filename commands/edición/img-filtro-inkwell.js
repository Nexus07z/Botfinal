...
module.exports = {
    name: "img-filtro-inkwell",
    alias: ["imgfinkwell"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro inkwell.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/inkwell", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/inkwell", { img: isUrl(text)[0] }, "apikey"), "", m)
