...
module.exports = {
    name: "img-filtro-valencia",
    alias: ["imgfvalencia"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro valencia.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/valencia", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/valencia", { img: isUrl(text)[0] }, "apikey"), "", m)
