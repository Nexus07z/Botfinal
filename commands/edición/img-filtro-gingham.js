...
module.exports = {
    name: "img-filtro-gingham",
    alias: ["imgfgingham"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro gingham.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/gingham", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/gingham", { img: isUrl(text)[0] }, "apikey"), "", m)
