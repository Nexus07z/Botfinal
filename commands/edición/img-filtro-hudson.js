...
module.exports = {
    name: "img-filtro-hudson",
    alias: ["imgfhudson"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro hudson.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/hudson", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/hudson", { img: isUrl(text)[0] }, "apikey"), "", m)
