...
module.exports = {
    name: "img-mascara-esqueleto",
    alias: ["imgskullmask"],
    use: "<Respuesta>",
    desc: "Agrega un mascara de esqueleto a una imagen.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/editor/skullmask", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/editor/skullmask", { img: isUrl(text)[0] }, "apikey"), "", m)
