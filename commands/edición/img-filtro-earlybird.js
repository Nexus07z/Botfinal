...
module.exports = {
    name: "img-filtro-earlybird",
    alias: ["imgfearlybird"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro earlybird.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/earlybird", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/earlybird", { img: isUrl(text)[0] }, "apikey"), "", m)
