...
module.exports = {
    name: "img-filtro-clarendon",
    alias: ["imgfclarendon"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro clarendon.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/clarendon", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/clarendon", { img: isUrl(text)[0] }, "apikey"), "", m)
