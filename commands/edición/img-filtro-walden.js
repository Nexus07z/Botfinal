...
module.exports = {
    name: "img-filtro-walden",
    alias: ["imgfwalden"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro walden.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/walden", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/walden", { img: isUrl(text)[0] }, "apikey"), "", m)
