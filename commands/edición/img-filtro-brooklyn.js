...
module.exports = {
    name: "img-filtro-brooklyn",
    alias: ["imgfbrooklyn"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro brooklyn.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/brooklyn", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/brooklyn", { img: isUrl(text)[0] }, "apikey"), "", m)
