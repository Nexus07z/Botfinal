...
module.exports = {
    name: "img-filtro-aden",
    alias: ["imgfaden"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro aden.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/aden", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/aden", { img: isUrl(text)[0] }, "apikey"), "", m)
