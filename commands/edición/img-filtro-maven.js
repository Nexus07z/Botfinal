...
module.exports = {
    name: "img-filtro-maven",
    alias: ["imgfmaven"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro maven.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/maven", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/maven", { img: isUrl(text)[0] }, "apikey"), "", m)
