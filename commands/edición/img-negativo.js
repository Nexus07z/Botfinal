...
module.exports = {
    ...
    alias: ["imgtoinvert"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con estilo negativo.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/editor/invert", { img: link }, "apikey"), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/editor/invert", { img: isUrl(text)[0] }, "apikey"), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
