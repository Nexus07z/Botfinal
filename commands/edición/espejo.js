...
module.exports = {
    ...
    alias: ["flip"],
    use: "<Respuesta>",
    desc: "Da la vuelta a una imagen.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/editor/flip", { img: link }, "apikey"), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/editor/flip", { img: isUrl(text)[0] }, "apikey"), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
