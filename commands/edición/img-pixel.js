...
module.exports = {
    ...
    alias: ["imgtopixel"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con estilo pixel.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/editor/pixelate", { img: link }, "apikey"), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/editor/pixelate", { img: isUrl(text)[0] }, "apikey"), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
