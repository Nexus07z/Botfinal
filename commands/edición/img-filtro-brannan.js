...
module.exports = {
    name: "img-filtro-brannan",
    alias: ["imgfbrannan"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro brannan.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/brannan", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/brannan", { img: isUrl(text)[0] }, "apikey"), "", m)
