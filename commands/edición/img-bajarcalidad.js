...
module.exports = {
    name: "img-bajarcalidad",
    alias: ["deepfry"],
    use: "<Respuesta>",
    desc: "Disminuye la calidad de una imagen.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/deepfry", { img: link }, "apikey"), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/deepfry", { img: isUrl(text)[0] }, "apikey"), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
