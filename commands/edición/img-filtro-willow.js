...
module.exports = {
    name: "img-filtro-willow",
    alias: ["imgfwillow"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro willow.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/willow", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/willow", { img: isUrl(text)[0] }, "apikey"), "", m)
