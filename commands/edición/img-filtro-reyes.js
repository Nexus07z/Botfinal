...
module.exports = {
    name: "img-filtro-reyes",
    alias: ["imgfreyes"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro reyes.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/reyes", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/reyes", { img: isUrl(text)[0] }, "apikey"), "", m)
