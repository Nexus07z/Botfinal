...
module.exports = {
    ...
    alias: ["imgtrash"],
    use: "<Respuesta>",
    desc: "Convierte una imagen en meme de basura.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/creator1/trash", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/creator1/trash", { img: isUrl(text)[0] }, "apikey"), "", m)
