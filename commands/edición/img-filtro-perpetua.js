...
module.exports = {
    name: "img-filtro-perpetua",
    alias: ["imgfperpetua"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro perpetua.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/perpetua", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/perpetua", { img: isUrl(text)[0] }, "apikey"), "", m)
