...
module.exports = {
    name: "img-filtro-rise",
    alias: ["imgfrise"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro rise.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/rise", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/rise", { img: isUrl(text)[0] }, "apikey"), "", m)
