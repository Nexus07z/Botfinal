...
module.exports = {
    name: "img-filtro-stinson",
    alias: ["imgfstinson"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro stinson.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/stinson", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/stinson", { img: isUrl(text)[0] }, "apikey"), "", m)
