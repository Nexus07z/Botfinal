...
module.exports = {
    name: "img-filtro-kelvin",
    alias: ["imgfkelvin"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro kelvin.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/kelvin", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/kelvin", { img: isUrl(text)[0] }, "apikey"), "", m)
