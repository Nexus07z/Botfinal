...
module.exports = {
    name: "img-filtro-mayfair",
    alias: ["imgfmayfair"],
    use: "<Respuesta>",
    desc: "Convierte una imagen con filtro mayfair.",
    ...
    start: async(killua, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/mayfair", { img: link }, "apikey"), "", m)
            ...
            try {
                ...
                await killua.sendFile(m.from, global.apilol("lol", "/filter/mayfair", { img: isUrl(text)[0] }, "apikey"), "", m)
