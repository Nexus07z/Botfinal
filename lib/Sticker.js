...
async function imageToWebp (media) {
    ...
    await new Promise((resolve, reject) => {
        ff(tmpFileIn)
            .on("error", reject)
            .on("end", () => resolve(true))
            .addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(240,ih)':force_original_aspect_ratio=increase,fps=15`]).toFormat('webp').save(tmpFileOut)
...
async function writeExifImg (media, metadata) {
    ...
    if (metadata.packname || metadata.author) {
        ...
        const json = { "sticker-pack-id": `https://github.com/Nexus07z/Botfinal`, "sticker-pack-name": metadata.packname, "sticker-pack-publisher": metadata.author, "emojis": metadata.categories ? metadata.categories : [""] }
